...
exports.getAllValidSchedules = function(callback) {
    ...
    getAllFromCacheOrDB(tableNames.scheduleTable,whereClause,tableNames.scheduleTableKey,callback);
...
exports.placeOrder = function(userInfo, orderInfo, confirmationCode, callback){
    ...
    async.waterfall([
            ...
            connection.query(selectSql,params, function(error,results){
                ...
                if(results && results.length>0) {
                    ...
                    callback1(null, results[0].customer_id);
                       ...
                       var insertCustomerSQL = "insert into " + tableNames + " ( customer_name, mobile_phone, email, is_registered ) " +
                           " VALUES ( ?,?,?,? )";
        ...
        function(customerId, callback2){
            ...
            var params = [customerId, orderInfo.totalAmount, confirmationCode, orderInfo.status, 'N/A'];

            connection.query(insertOrderSQL,params, function(error,results){
                if(error) { console.warn("create order failed!"); return; }
    ...
    var userKey =[userInfo.customerName, userInfo.mobilePhone];
    var selectCustomer = " select customer_id from "+ tableNames.customerTable + "where customer_name=? and mobile_phone=? ";

    var userData=[userInfo.customerName, userInfo.mobilePhone, userInfo.email, 'N'];
    var insertCustomerSql = "insert into " + tableNames.customerTable + " (customer_name, mobile_phone, email, is_registered) values " +
        " (?,?,?,?) ";
    ...
    var orderData=[customerId, orderInfo.totalOrderAmount, confirmationCode, orderInfo.orderStatus, orderInfo.description];
    ...
    var vouchersArray = orderInfo.vouchers;
    console.log("o_length=" + vouchersArray.length);
    for(var i=0;i<vouchersArray.length;++i) {
        var voucherInfo=[];
        console.log("i=" + i);
        voucherInfo[0]=orderId;
        voucherInfo[1]=vouchersArray[i].offerId;
        voucherInfo[2]=vouchersArray[i].skuId;
        voucherInfo[3]=vouchersArray[i].scheduleId;
        voucherInfo[4]=vouchersArray[i].quantity;
        voucherInfo[5]=vouchersArray[i].valid_date;
        voucherInfo[5]=vouchersArray[i].offerQuantity;
        voucherInfo[5]=vouchersArray[i].offerSubtotalAmount;
        voucherInfo[5]=vouchersArray[i].voucherDescription;
