 ...
 * @param callback   return callback, returns an array of toolIds which the user has access
 */
exports.getToolIdsFromUsername = function(username, callback) {
    var sql = " select tool_id from " + tableNames.adminToolPermissionTable + " where username = ? ";
    runQueryWithParams(sql,[username], function(err,results){
        if(err) {
            throw err;
        }
 ...
 *
 * @param username     username for the user
 * @param toolIdList   an Array of toolIds
 * @param callback     'done' / 'failed'
 */
exports.addPermissionsToUser = function(username, toolIdList, callback) {
    ...
    for(var i in toolIdList) {
        var toolId = toolIdList[i];
        var params=[toolId, username];//voucher.voucherDescription];
 ...
 * @param callback     'done' / 'failed'
 */
exports.removePermissionsFromUser = function(username, toolIdList, callback) {

    var removePermissionSql = " delete " + tableNames.adminToolPermissionTable + " where tool_id = ? and username = ? " ;
    //Add all the permissions as a batch. Rollback if any one is failed
    var hasError = false;
    var trans = connection.startTransaction();
    ...
    function error(err) {
        if(err && trans.rollback) {
            hasError = true;
            trans.rollback();
            console.error("remove tool permissions from user failed, rolling back!");
            throw err;
        }
    }
