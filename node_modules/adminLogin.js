...
function verifyUser(username, randomKey, callback) {
    var verifySql = " select  randomKey from " + tableNames.adminLoginHistoryTable  +
        " where username = ? and login_datetime > now() - INTERVAL 4 HOURS ";

    dbPool.runQueryWithParams(sql, [username], function(err, results){
        if(err) {
            callback(err,null);
            return;
        }
        if(results && results.length==1) {
            ...
            if(results[0].random_key == randomKey) {
